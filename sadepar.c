...
void sa_printBestSolution(sa_t *t, sa_state *state, int* bestIdx, float seconds,double bfsenergy, double bfssmape, double bfsDA) {
	FILE *solFile2 = fopen("walmex.reg","a");
	int i,j;
	printf("Solution: \n");
...
void ts_print(sa_t *t, sa_state *state,timeserie *serie, int bestIdx, int start, float seconds){
	...
	FILE *solFile = fopen("walmex.out","w");
...
/*
  ...
  //srand(127);
	...
	t = sa.t0;
	if(id==0){
		sol = (double *)malloc(p*sa.dim*sizeof(double));
	}
	for(i=0; i < sa.populationSize ; i++){
		for(j=0;j<sa.dim;j++){
			S.solution[j]=current.solution[i][j];
	...
	printf("Time: %lf\n\n",seconds);
	MPI_Gather(current.solution[bestSolIdx], sa.dim, MPI_DOUBLE, sol, sa.dim, MPI_DOUBLE, 0,MPI_COMM_WORLD);
	//if(id==0){
